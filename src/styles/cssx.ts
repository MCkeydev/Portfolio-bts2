...
import { TIndexedObject } from '../types/TIndexedObject';

export type TCssxProperties = {
    [key in keyof CSSProperties]: ((theme: Theme) => string|number)|string|number;
};
...
const cssxShortcuts: (theme: Theme) => ICssxShortcuts = (theme: Theme) => ([
    ...
    {
        keys: [
            'padding', 'paddingTop', 'paddingRight', 'paddingBottom', 'paddinfLeft',
 ...
 * @returns css object
 */
const styleFunctionCssx = (cssx: TIndexedObject, theme: Theme): Interpolation<Theme> => {
 ...
 */
const cssx = (cssx: TCssxProperties) => (theme: Theme) => styleFunctionCssx(cssx, theme);
